...
-- Create the enhanced function with the correct signature
...
BEGIN
  ...
  CASE old_role
    WHEN 'doctor' THEN
      -- Remove from doctors table (cascade will handle related records)
      DELETE FROM doctors WHERE id = p_user_id;
    WHEN 'patient' THEN
      -- Remove from patients table (cascade will handle related records)
