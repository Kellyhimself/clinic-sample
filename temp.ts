...
import type { Patient, UnpaidAppointment, Sale } from '@/types/supabase';

const supabase = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);

// Create an extended Patient interface that includes email
interface PatientWithEmail extends Patient {
  id: string;
  full_name: string;
        ...
        try {
          ...
          if (salesError) {
            ...
            const transformedSales = sales?.map(sale => {
              // Ensure patient is an object and not an array
              const patientObj = Array.isArray(sale.patient) ? sale.patient[0] : sale.patient;
