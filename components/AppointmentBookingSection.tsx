...
export default function AppointmentBookingSection({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // No need to modify the patientId here - our backend handles it properly
      const patientId = selectedPatient.id;

      // Use the handleBooking function with options for staff booking
            ...
            />
              ...
              <div className="flex justify-between items-center">
                ...
                {step < 3 ? (
                  <Button
                    onClick={() => setStep(step + 1)}
                    disabled={(step === 1 && !selectedPatient) || (step === 2 && (!selectedService && !customService || !selectedDoctor))}
                    size="sm"
