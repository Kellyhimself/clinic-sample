<div className="space-y-4 inventory-container">
  <Card className="bg-white/80 backdrop-blur-sm">
    <CardHeader className="p-2 sm:p-4 bg-gradient-to-r from-blue-500/10 to-teal-500/10 rounded-t-lg">
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <CardTitle className="text-base sm:text-lg text-gray-800">Inventory Management</CardTitle>
      </div>
    </CardHeader>
    <div className="p-2 sm:p-4 md:p-6">
      {/* Add usage limit alert */}
      {shouldShowAlert('inventory') && !dismissedAlerts.has('inventory') && (
        <UsageLimitAlert
          featureId="inventory"
          tenantId={tenantId}
          currentUsage={getLimit('inventory')?.current || 0}
          limit={getLimit('inventory')?.limit || 0}
          type={getLimit('inventory')?.type || 'warning'}
          onDismiss={() => setDismissedAlerts(prev => new Set([...prev, 'inventory']))}
        />
      )}
        ...
        <div className="p-2 sm:p-4 border-b flex flex-col sm:flex-row gap-2 sm:gap-4 bg-gradient-to-r from-gray-50 to-blue-50/50">
          ...
          <input
            ...
            placeholder={`Filter by ${filterType} (e.g., ${
              filterType === 'name' ? 'Aspirin' : filterType === 'category' ? 'Pain Relief' : 'In Stock'
            })`}
            value={filterValue}
