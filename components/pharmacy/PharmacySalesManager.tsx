...
export default function PharmacySalesManager() {
  ...
  const saleColumns = [
    ...
    },
    {
      header: 'Total',
      key: 'total',
      cell: (sale: Sale) => `KSh ${calculateTotal(sale).toFixed(2)}`
    },
    {
      header: 'Payment',
      key: 'payment',
  ...
  return (
    <div className="space-y-3 p-2 md:space-y-6 md:p-4 lg:p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
        <SalesMetricCard
          title="Total Revenue"
          value={new Intl.NumberFormat('en-KE', { style: 'currency', currency: 'KES' }).format(salesAnalytics.totalRevenue)}
          icon={<Banknote className="h-3.5 w-3.5 md:h-4 md:w-4 text-blue-600" />}
          subValue={`${salesAnalytics.totalSales} total sales`}
        />
        
        <SalesMetricCard
          title="Average Sale"
          value={new Intl.NumberFormat('en-KE', { style: 'currency', currency: 'KES' }).format(salesAnalytics.averageSaleValue)}
          icon={<TrendingUp className="h-3.5 w-3.5 md:h-4 md:w-4 text-green-600" />}
          subValue={`${salesAnalytics.totalItems} items sold`}
        ...
        <SalesMetricCard
          ...
          subValue={salesAnalytics.paidSales > 0 ? ((salesAnalytics.paidSales / salesAnalytics.totalSales) * 100).toFixed(1) + '% of total' : '0%'}
        ...
        <SalesMetricCard
          ...
          subValue={salesAnalytics.pendingSales > 0 ? ((salesAnalytics.pendingSales / salesAnalytics.totalSales) * 100).toFixed(1) + '% of total' : '0%'}
      ...
      <div className="flex overflow-x-auto -mx-2 px-2 pb-2 snap-x snap-mandatory md:mx-0 md:px-0 md:snap-none md:flex-wrap md:gap-3 md:mb-4">
        ...
        <Button
          variant="outline"
          size="sm"
          className="flex-shrink-0 mr-2 snap-start min-w-[140px] flex items-center gap-1 text-[10px] md:text-sm bg-gradient-to-r from-rose-50 to-rose-100 border-rose-200 text-rose-700 hover:bg-rose-200 h-8"
      ...
      {/* Make the NewSaleForm take full screen on mobile */}
      {showNewSaleForm && (
